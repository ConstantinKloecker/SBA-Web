(function () {
  function init() {
    ...
    // Also clone the inline "Get the app" CTA into the dropdown as an
    // emphasized item (only visible inside the open panel).
    var cta = navLinks.querySelector('.btn');
    if (cta) {
      var ctaClone = cta.cloneNode(true);
      ctaClone.classList.remove('btn', 'btn-small');
      ctaClone.classList.add('nav-more-item', 'nav-more-cta');
      ctaClone.setAttribute('role', 'menuitem');
